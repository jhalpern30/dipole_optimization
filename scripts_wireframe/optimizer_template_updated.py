...
for r in TF_r_values:
    ...
    if fixed_geo: # need to do this so that there is something to optimize for fixed geo cases
    # could also just make sure it never enters these loops
...
# Analysis to find the optimal TF coil radius where the change in J is less than 10% cause J decreases monotonically
for i in range(1, len(J_values)):
    if abs(J_values[i] - J_values[i-1]) / J_values[i-1] < 0.1:
        TF_R1 = TF_r_values[i]
...
fig.suptitle(f'Optimization of TF radius with VV radius = {VV_a:.3f} [m]', fontsize=titlefontsize, fontweight='bold')
...
for i, (dof_type, indices) in enumerate(dof_types.items(), start=1):
    ...
    if dof_type == 'x0':
        ...
        dof_type = 'R0 [m]'
    elif dof_type == 'r_rotation':
        dof_type = 'r rotation [rad]'
...
# Note: this function could use improvement, but should work fine for now
# Main issues: currents/lambda relationship is highly nonlinear, currents change quickly over a small range of lambda
# Difficult to find algorithm that goes quickly over range where gradients are small, but doesn't jump over large gradients
# Might be helpful to plot max current vs. max lambda for a few runs, look for characteristics/better scale factors
def update_Lambda(Lambda):
    ...
    scale_fac = 0.05
    if wf_max > max_I:
        Lambda *= (np.abs(wf.currents) / max_I)**0.5 + scale_fac # slow this down a bit with the sqrt
        # normalize
        Lambda *= np.sqrt(np.sum(Lambda_initial**2 * wf.currents**2) / np.sum(Lambda**2 * wf.currents**2))
    # If all currents are below the tolerance (i.e. too large a jump occurred), scale down entire array
    elif wf_max < max_I * (1 - cur_tol):
        print("all currents too low")
        Lambda *= wf_max / max_I
    return Lambda

def has_leveled_out(prev_max_currents, current_max_current, convergence_threshold=5e2): # break if diff < convergence_threshold [Amps] over 5 iterations
    if len(prev_max_currents) > 5:
        # Calculate the differences between the current maximum and previous maximums
        diffs = [abs(current_max_current - prev_max) for prev_max in prev_max_currents[-5:]]
        return all(diff < convergence_threshold for diff in diffs)
    return False

def fun_wf(lambda_init):
    params = {'reg_lambda': 10**lambda_init, \
    ...
    print(f"    Max current = {wf_max:.3e}, setting lambda_init = {lambda_init:.2f}")
...
if wf_max < max_I:
    ...
    res = minimize_scalar(fun_wf, bounds=(-12, -6), method='bounded', tol=1e-2)
    lambda_init = res.x
    diff = 0.25 # hacking this for now
    # initialize Lambda array to a scalar slightly below where the max current is hit
    # this allows the high currents to be decreased and low currents to be decreased
    # similar to the function that starting ratio served
    Lambda = np.ones(wf.nSegments) * 10**(lambda_init - diff)
    Lambda_initial = Lambda.copy()
    ...
    while wf_max > max_I or wf_max < max_I * (1 - cur_tol):
        ...
        if has_leveled_out(prev_max_currents, wf_max):
            print(f"     Max WF current has leveled out. Try again with a smaller diff (need to code this up at some point)")
            Lambda = update_Lambda(Lambda)
            if len(prev_max_currents) > 6:
                prev_max_currents.pop(0)  # Keep only the last 6 values to check leveling off 
        else:
            Lambda = update_Lambda(Lambda)
        res = optimize_wireframe_wrapper(Lambda)
        prev_max_currents.append(wf_max)
        wf_max = np.abs(wf.currents[wf.unconstrained_segments()]).max().copy()
...
while (JF_i - JF.J()) / JF_i > JF_tol: # keep looping through TF/WF optimization, making sure currents are less than max_I as needed
    ...
    print(JF.x)
    wf_max = np.abs(wf.currents[wf.unconstrained_segments()]).max().copy()
    while field_flag and (wf_max > max_I * (1 + cur_tol)): 
        print(f'     Maximum WF Current = {wf_max:.3f}, Maximum Allowed Current = {max_I:.3f}, Maximum Lambda Value = {Lambda[wf.unconstrained_segments()].max():.3e}')
        Lambda = update_Lambda(Lambda)
        res = optimize_wireframe_wrapper(Lambda)
        wf_max = np.abs(wf.currents[wf.unconstrained_segments()]).max().copy()
